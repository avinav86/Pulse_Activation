...
TARGET = "https://procivic-addyson-kaleidoscopically.ngrok-free.dev"
...
async def run_session(playwright, session_num, ua, viewport, scenario_name):
    ...
    try:
        ...
        print(f"  [OK] Session {session_num:02d} | {scenario_name:<18} | {ua[50:90]}...")
    except Exception as e:
        print(f"  [!!] Session {session_num:02d} error: {e}")
    finally:
        await context.close()
        await browser.close()
...
async def main():
    print(f"\nPULSE Fashion — Conviva test traffic simulator")
    print(f"Target : {TARGET}")
    print(f"Sessions: 30  |  Concurrency: 3\n")
    ...
    async with async_playwright() as playwright:
        for batch_start in range(0, 30, 3):
            ...
            print(f"Batch {batch_start // 3 + 1}/10  (sessions {batch[0]['num']}-{batch[-1]['num']})")
    ...
    print("\nDone — 30 sessions completed. Check your Conviva dashboard.")
